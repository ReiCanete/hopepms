...
export default function EditProductModal({ product, onClose, onSuccess }) {
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    setLoading(true);
    const { error } = await updateProduct({ prodCode: product.prodCode, ...form, userId: currentUser.id });
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md">
        <div className="flex items-center justify-between p-6 border-b border-slate-100">
          <div>
            ...
            <p className="text-xs text-slate-400 font-mono mt-0.5">{product.prodCode}</p>
          </div>
